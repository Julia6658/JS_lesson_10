...
const heroes = [{
  ...
  const heroesString = JSON.stringify(heroes);
  const heroesObj = JSON.parse(heroesString);
  
  const heroesWrap = document.querySelector("#heroes-wrap");
  
  heroesObj.forEach((item) => {
    let heroRating;
    
    const heroItem = document.createElement("div");
    heroItem.classList.add('hero-item');
    heroesWrap.append(heroItem);
  
    // переменные для разметки
    const heroName = document.createElement("div");
    const heroUniverse = document.createElement("div");
    const heroAlterego = document.createElement("div");
    const heroOccupation = document.createElement("div");
    const heroFriends = document.createElement("div");
    const heroSuperpowers = document.createElement("div");
    ...
    // создание разметки
    heroItem.append(heroName);
    heroItem.append(heroUniverse);
    heroItem.append(heroAlterego);
    heroItem.append(heroOccupation);
    heroItem.append(heroFriends);
    heroItem.append(heroSuperpowers);
    heroItem.append(heroInfo);
    heroItem.append(heroImg);
    heroItem.append(rating);
  
    // наполнение из массива
    heroName.innerHTML = `<b>Имя:</b> ${item.name}`;
    heroUniverse.innerHTML = `<b>Вселенная:</b>  ${item.universe}`;
    heroAlterego.innerHTML =  `<b>Альтерэго:</b>  ${item.alterego}`;
    heroOccupation.innerHTML = `<b>Род деятельности:</b> ${item.occupation}`;
    heroFriends.innerHTML = `<b>Друзья:</b> ${item.friends}`;
    heroSuperpowers.innerHTML = `<b>Суперсилы:</b> ${item.superpowers}`;
    heroInfo.innerHTML = `<b>Описание:</b> ${item.info}`;
    heroImg.setAttribute('src', item.url);
    rating.innerHTML = `
          <span title="1"><img src="star_empty.svg">
          </span><span title="2"><img src="star_empty.svg">
          </span><span title="3"><img src="star_empty.svg"></span>
          <span title="4"><img src="star_empty.svg"></span>
          <span title="5"><img src="star_empty.svg"></span>` ;
  
    // рейтинг     
    const ratingStars = heroItem.querySelectorAll(".hero-rating > span");
    ratingStars.forEach(function(star, index) {
      star.addEventListener('click', function() {
         
          heroRating = star.getAttribute('title');
          item.rating = heroRating;
          star.classList.add('clicked');
          star.parentElement.classList.add('disabled', 'disabled' + star.getAttribute('title'));
          
          console.log("Рейтинг героя " + item.rating);
          window.localStorage.setItem('rate', item.rating)
      });
